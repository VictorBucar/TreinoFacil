...
namespace TreinoFacil.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult RegisterProf()
        {
            return View();
        }

        //
        // POST: /Account/RegisterProf
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterProf(RegisterProfViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    await UserManager.AddToRoleAsync(user.Id, "Prof");
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
